...
class AsymKD_DPTHead_visualize(nn.Module):
    def __init__(self, nclass, in_channels, features=128, use_bn=False, out_channels=[96, 192, 384, 768], use_clstoken=False):
        super(AsymKD_DPTHead_visualize, self).__init__()
    ...
    def forward(self, depth_intermediate_features, depth_patch_h, depth_patch_w, seg_intermediate_features, seg_patch_h, seg_patch_w ):
        ...
        for i, x in enumerate(depth_intermediate_features):
            ...
            x = x.permute(0, 2, 1).reshape((x.shape[0], x.shape[-1], depth_patch_h, depth_patch_w))
            depth_feature_out.append(x)

        seg_resize_out = []
        for idx, seg_feature in enumerate(seg_intermediate_features):
            seg_feature = F.interpolate(seg_feature, size=(depth_feature_out[idx].shape[2]*2, depth_feature_out[idx].shape[3]*2), mode='bilinear', align_corners=False)
            seg_feature = F.max_pool2d(seg_feature, kernel_size=2)
            seg_resize_out.append(seg_feature)
        ...
        '''Cross Attention 연산 코드'''
        Cross_Attention_Features = []
        for Cross_Attention_Blocks, Blocks,depth_feature, seg_feature in zip(self.Cross_Attention_blocks_layers, self.Blocks_layers, depth_feature_out, seg_resize_out):
            batch_size, channels, height, width = depth_feature.shape
            reshaped_depth_feature = depth_feature.reshape(batch_size, channels, height * width).permute(0, 2, 1)
            reshaped_seg_feature = seg_feature.reshape(batch_size, channels, height * width).permute(0, 2, 1)
            feature = Cross_Attention_Blocks(reshaped_depth_feature, reshaped_seg_feature)
        ...
        return depth_out, depth_feature_out, seg_resize_out, Cross_Attention_Features
...
class AsymKD_DepthAnything_visualize(nn.Module):
    def __init__(self, ImageEncoderViT, features=128, out_channels=[96, 192, 384, 768], use_bn=False, use_clstoken=False, localhub=True):
        super(AsymKD_DepthAnything_visualize, self).__init__()
        ...
        self.depth_head = AsymKD_DPTHead_visualize(1, dim, features, use_bn, out_channels=out_channels, use_clstoken=use_clstoken)
    ...
    def forward(self, depth_image,seg_image):
        ...
        depth, depth_feature_out, seg_resize_out, Cross_Attention_Features = self.depth_head(depth_intermediate_features, depth_patch_h, depth_patch_w, seg_intermediate_features, seg_patch_h, seg_patch_w )
        depth = F.interpolate(depth, size=(depth_patch_h*14, depth_patch_w*14), mode="bilinear", align_corners=True)
        depth = F.relu(depth)
        # depth = self.nomalize(depth)
        return depth, depth_feature_out, seg_resize_out, Cross_Attention_Features
